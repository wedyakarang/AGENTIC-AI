...
//
// PERBEDAAN DARI VERSI OPENAI: hanya bagian pemanggilan LLM
// (callGeminiWithRetry, callGeminiForceTool, buildUsagePayload,
// schema tool pakai SchemaType Gemini). SEMUA logika filter/hitung
// (applyFilter, formatCountAnswer, formatListAnswer,
// formatNameSearchAnswer, formatAggregateAnswer) TIDAK DIUBAH SAMA
// SEKALI - tetap 100% JS deterministik seperti sebelumnya, karena
// itu justru inti kenapa file ini didesain begini (lihat komentar
// asli di bawah).
//
// KENAPA DIDESAIN BEGINI (penting):
// Versi awal mengirim SELURUH data mentah ke AI dan minta AI
...
// selama data sumbernya (fetchAllPromotions) benar.
//
// v25 - parameter "name_query" untuk PENCARIAN NAMA SPESIFIK vs
// PERMINTAAN AGREGAT.
// v26 - parameter "is_relevant" untuk menangani pertanyaan OFF-TOPIC.
// v27 - answer_type "aggregate" + aggregate_field & aggregate_function.
//
// CATATAN ARSITEKTUR: executorAgent.js (versi Gemini) juga sudah
// dikonversi terpisah. Modul ini titik LLM KEDUA (khusus command
// /analisis), tetap terpisah dari executorAgent.js.
// ==========================================================
...
// v27 - answer_type "aggregate" + aggregate_field & aggregate_function.
//
// v28 - FIX BUG: pertanyaan soal "max use" (batas maksimal pemakaian,
// field max_use) sebelumnya TIDAK PUNYA parameter filter sendiri -
// yang ada cuma used_comparison/used_value yang mewakili field
// "used" (pemakaian AKTUAL), bukan "max_use" (KUOTA/batas). Akibatnya
// pertanyaan seperti "promo yang max use-nya di bawah 10" tidak
// pernah benar-benar difilter - used_comparison jatuh ke "none" dan
// applyFilter() meloloskan SEMUA data. Sekarang ditambahkan
// max_use_comparison + max_use_value, mengikuti pola yang sama
// persis dengan used_comparison/used_value, supaya AI punya "kotak
// isian" yang tepat untuk filter berbasis max_use.
//
// CATATAN ARSITEKTUR: executorAgent.js (versi Gemini) juga sudah
// dikonversi terpisah. Modul ini titik LLM KEDUA (khusus command
// /analisis), tetap terpisah dari executorAgent.js.
...
// CEK nama model Gemini terbaru yang tersedia di akun kamu - placeholder.
const MODEL = "gemini-3.1-flash-lite";

// Padanan reasoning_effort: "none" - mematikan extended thinking.
...
// Dipertahankan sebagai jaga-jaga kalau suatu saat args berupa string.
...
// TOOL: query_promotions - AI HANYA menentukan FILTER & JENIS
// JAWABAN yang diminta user, TIDAK menghitung apapun sendiri.
// Schema pakai SchemaType Gemini, bukan JSON Schema biasa.
// ==========================================================
const QUERY_PROMOTIONS_FUNCTION = {
  name: "query_promotions",
  description:
...
const QUERY_PROMOTIONS_TOOLSET = { functionDeclarations: [QUERY_PROMOTIONS_FUNCTION] };

// ==========================================================
// PENGHITUNGAN/FILTER DATA - 100% JS, DETERMINISTIK, TIDAK PERNAH
// SALAH HITUNG (selama data sumber dari GuestPro benar).
// v28: ditambahkan blok filter max_use_comparison/max_use_value,
// mengikuti pola persis blok used_comparison/used_value di
// bawahnya. Tidak ada logika lain yang diubah.
// ==========================================================
function applyFilter(promotions, args) {
  const nameQuery = (args.name_query || "").trim().toLowerCase();
  ...
  return promotions.filter(p => {
    ...
    // FIX v28: filter berdasarkan max_use (batas maksimal/kuota),
    // sebelumnya tidak ada sama sekali sehingga pertanyaan seperti
    // "max use di bawah 10" tidak pernah benar-benar difilter.
...
function describeFilter(args) {
  ...
  // FIX v28: deskripsi filter max_use, biar muncul juga di
  // header jawaban count/list, bukan cuma dipakai buat filter diam-diam.
  if (args.max_use_comparison && args.max_use_comparison !== "none") {
...
async function answerPromotionQuestion({ chatId, processId, question }) {
  ...
  const model = genAI.getGenerativeModel({
    model: MODEL,
    tools: [QUERY_PROMOTIONS_TOOLSET],
    toolConfig: {
      functionCallingConfig: {
        mode: "ANY", // padanan tool_choice: "required"
        allowedFunctionNames: ["query_promotions"]
      }
    },
    generationConfig: {
      maxOutputTokens: 500,
  ...
  console.log("🔍 usage mentah dari Gemini (analisis-ai):", JSON.stringify(usageMetadata));
  ...
  const call = calls.find(c => c.name === "query_promotions");
