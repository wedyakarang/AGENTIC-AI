...
//
// Jalur:
// 1. finalizeAndCreate() — Manual. AI SELALU dipanggil untuk setiap
//    promotion. AI mengecek HANYA 2 hal: nama promotion terisi, dan
//    SEMUA kode promo (promoCodes[].kode) terisi. Kalau keduanya
//    terisi -> AI panggil create_promotion. Kalau ada yang kosong ->
//    AI panggil flag_anomaly (sebut field mana yang kosong), data
//    DITAHAN sampai user mengisi lewat tombol Edit.
// 2. forceCreateAfterFlag() / cancelAfterFlag() — generalized,
//    menangani flag type "manual" DAN "import".
//    - Sebelum eksekusi, SELALU dicek ulang (isFlagResolved) apakah
//      nama & semua kode promo BENAR-BENAR sudah terisi - kalau masih
//      ada yang kosong, eksekusi DITOLAK (bukan cuma soal tombol UI,
//      tapi validasi ulang di server).
//    - "manual": TIDAK MEMANGGIL AI SAMA SEKALI setelah lolos guard -
//      data sudah pasti lengkap & user sudah konfirmasi, tidak perlu
//      ditimbang ulang.
//    - "import": AI dipanggil TEPAT SATU KALI untuk SELURUH batch
//      baris yang tadinya kosong (lihat executeRowsBatchViaAI),
//      BUKAN per baris - supaya hemat token walau baris banyak.
// 3. finalizeImportExcel() — rule-based murni (if/else, 0 token) yang
//    MEMUTUSKAN per baris: nama & kode promo terisi semua = DATA AMAN,
//    ada yang kosong = DATA KOSONG. Baris DATA AMAN -> AI tetap
//    dipanggil, tapi HANYA SATU KALI untuk SELURUH batch baris aman
//    (bukan per baris) - lihat executeRowsBatchViaAI. Baris DATA
//    KOSONG -> AI dipanggil SATU KALI untuk seluruh batch kosong itu
//    juga, tugasnya cuma bikin resume/ringkasan kenapa kosong (BUKAN
//    eksekusi - eksekusi baru terjadi setelah semua field di batch itu
//    diisi lewat tombol Edit lalu user tekan "Tetap Lanjutkan").
//
// ==========================================================
//
// VALIDASI (KELENGKAPAN NAMA PROMOTION & KODE PROMO):
//
// Validasi HANYA soal kelengkapan data - apakah field NAMA PROMOTION
// dan KODE PROMO (promoCodes[].kode) terisi atau tidak:
// - Terisi semua -> "DATA AMAN", boleh langsung dieksekusi/lanjut.
// - ADA yang kosong -> "DATA KOSONG", WAJIB diisi dulu lewat tombol
//   Edit sebelum bisa lanjut - BUKAN pilihan "tetap lanjutkan apa
//   adanya", karena data kosong memang tidak valid untuk dikirim ke
//   GuestPro.
//
// Konsekuensi UI: tombol yang muncul otomatis menyesuaikan status
// TERKINI data (dicek ulang tiap kali pesan di-render):
// - Masih ada field kosong -> tombol "✏️ Edit Data" + "❌ Batalkan".
// - Semua field sudah terisi (baru selesai diedit) -> tombol berubah
//   jadi "✅ Lanjutkan" + "❌ Batalkan".
// Fungsi isFlagResolved() di bawah adalah SATU SUMBER KEBENARAN untuk
// menentukan status ini, dipakai baik oleh buildFlagKeyboard (render
// tombol) MAUPUN forceCreateAfterFlag (guard sebelum eksekusi nyata) -
// supaya tombol basi/ter-cache tidak bisa memaksa data kosong lolos.
//
// ==========================================================
//
// PATCH TOKEN-SAVING (tetap dipertahankan):
//
// 1. reasoning_effort: "none" di SEMUA panggilan OpenAI - model
//    reasoning WAJIB pakai "none" kalau dipakai bareng `tools`
//    (endpoint /v1/chat/completions menolak selain itu), dan itu pas
//    karena tugas di sini cuma cek 2 field / merangkum / memanggil
//    tool - tidak butuh reasoning panjang.
//
// 2. Jalur Import tetap: AI dipanggil TEPAT SATU KALI per batch
//    (bukan per baris) - baik untuk resume batch kosong, maupun untuk
//    eksekusi batch aman/batch yang sudah lengkap setelah diedit.
//
// ==========================================================
